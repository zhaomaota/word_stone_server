### **1. 用户表（users）**

- 用户ID（主键）
- 用户名（唯一）
- 密码哈希
- 总鲜花数（别人送给我的花）
- 累计开包数
- 连续签到天数
- 累计在线时长（分钟）
- 我的邀请码（唯一）
- 我是被谁邀请的（外键指向另一个用户）
- 注册时间
- 最后登录时间
- 最后签到时间
- 等级（还没想好有什么用，先留着）
- 是否封禁

### **2. 单词表（words）**

- 单词ID（主键）
- 单词本身（唯一，比如 "apple"）
- 中文释义（比如 "苹果"）
- 稀有度（common/rare/epic/legendary）
- 发音（音标或语音文件URL）
- 单词变体（包含多个元素的数组，元素是单词的复数形式或其他时态等变体）
- 创建时间
- 词性


---

### **3. 用户单词库存表（user_words）**

- 用户ID（外键）
- 单词ID（外键）
- 是否收藏到生词本
- 获得时间


### **4. 卡包表（packs）**

- 卡包ID（主键）
- 卡包名称（比如 普通卡包或者传说卡包）
- 卡包描述
- 每包多少张卡（比如5张）
- 稀有度权重（JSON格式，比如 {"common": 70, "rare": 20, "epic": 8, "legendary": 2}）
- 是否启用（可以下架某些卡包）
- 创建时间


### **5. 用户卡包库存表（user_packs）**

记录"用户拥有多少个某种卡包"。

**核心字段：**
- 用户ID（外键）
- 卡包ID（外键）
- 数量

**联合主键：**
- （用户ID + 卡包ID）



---

### **6. 消息表（messages）**

- 消息ID（主键，可以用时间戳+随机数）
- 用户ID（外键，谁发的）
- 消息内容
- 鲜花数（这条消息收到多少花）
- 时间戳（毫秒级）
- 引用消息ID（表示引用了哪条别的消息）


【消息只保留最近24小时的】
---

### **7. 送花记录表（rose_senders）**

记录"谁给哪条消息送了花"，防止重复送花。

- 消息ID（外键）
- 送花人ID（外键）
- 送花时间
**联合主键：**
- （消息ID + 送花人ID）确保一个人只能给一条消息送一次花

### **8. 图鉴表（collections）**

**核心字段：**
- 图鉴ID（主键）
- 图鉴名称（比如 "动物图鉴"）
- 描述
- 图标
- 完成奖励 （卡包）
- 创建时间

---

### **9. 图鉴单词关联表（word_collections）**

记录"哪些单词属于哪个图鉴"。

**核心字段：**
- 图鉴ID（外键）
- 单词ID（外键）

**联合主键：**
- （图鉴ID + 单词ID）【一个单词可以属于多个图鉴】

---

### **10. 用户图鉴完成记录表（user_collections）**

记录"用户完成了哪些图鉴"。

**核心字段：**
- 用户ID（外键）
- 图鉴ID（外键）
- 完成时间

**联合主键：**
- （用户ID + 图鉴ID）



---

### **11. 成就表（achievements）**

**核心字段：**
- 成就ID（主键）
- 成就名称（比如 "鲜花达人"）
- 描述
- 图标（可选）
- 成就类别（比如"收集类"、"社交类"）
- 是否是隐藏成就
**解锁条件：**
- 条件类型（比如 "total_roses"、"vocab_count"、"consecutive_days"）
- 条件值（比如100）

**奖励：**
- 奖励鲜花数
- 奖励卡包（JSON格式，可选）

**时间戳：**
- 创建时间

### **12. 用户成就记录表（user_achievements）**

记录"用户解锁了哪些成就"。

**核心字段：**
- 用户ID（外键）
- 成就ID（外键）
- 解锁时间

**联合主键：**
- （用户ID + 成就ID）

---

### **13. 签到奖励记录表（login_rewards）**

记录"用户每次签到的奖励"。

**核心字段：**
- 记录ID（主键）
- 用户ID（外键）
- 第几天（连续签到的天数）
- 奖励鲜花数
- 奖励卡包（JSON格式，可选）
- 领取时间

好的！让我帮你梳理所有需要的接口。

---

## 🌐 接口设计总览

### **技术选型建议：**
- **HTTP 接口**：用于数据查询、状态修改（RESTful API）
- **WebSocket 接口**：用于实时聊天、在线状态、实时通知

---

## 📡 HTTP 接口（REST API）

### **1. 用户认证模块**

#### **POST /api/auth/register**
注册新用户
```json
请求：
{
  "username": "testuser",
  "password": "password123",
  "inviteCode": "ABC123" // 可选
}

响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "username": "testuser",
    "inviteCode": "XYZ789",
    "token": "jwt_token"
  }
}
```

#### **POST /api/auth/login**
用户登录
```json
请求：
{
  "username": "testuser",
  "password": "password123"
}

响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "username": "testuser",
    "token": "jwt_token",
    "user": {
      "totalRoses": 100,
      "level": 5,
      "consecutiveDays": 3
    }
  }
}
```

#### **POST /api/auth/logout**
用户登出
```json
请求：
{
  "token": "jwt_token"
}

响应：
{
  "success": true
}
```

#### **GET /api/auth/verify**
验证 Token 是否有效
```json
请求头：
Authorization: Bearer jwt_token

响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "username": "testuser"
  }
}
```

---

### **2. 用户信息模块**

#### **GET /api/users/me**
获取当前用户信息
```json
响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "username": "testuser",
    "totalRoses": 100,
    "totalPacks": 50,
    "consecutiveDays": 3,
    "level": 5,
    "inviteCode": "ABC123",
    "registeredAt": "2025-12-01T10:00:00Z",
    "isBanned": false
  }
}
```

#### **GET /api/users/:userId**
获取指定用户信息（公开信息）
```json
响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "username": "testuser",
    "totalRoses": 100,
    "level": 5,
    "registeredAt": "2025-12-01T10:00:00Z"
  }
}
```

#### **PUT /api/users/me**
更新当前用户信息
```json
请求：
{
  "avatar": "https://...",  // 可选
  "bio": "Hello world"      // 可选
}

响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "username": "testuser",
    "avatar": "https://...",
    "bio": "Hello world"
  }
}
```

---

### **3. 单词模块**

#### **GET /api/words**
获取单词列表（支持筛选）
```json
请求参数：
?rarity=rare          // 按稀有度筛选
&search=apple         // 搜索单词
&page=1               // 分页
&limit=20             // 每页数量

响应：
{
  "success": true,
  "data": {
    "words": [
      {
        "wordId": "uuid",
        "word": "apple",
        "definition": "苹果",
        "rarity": "common",
        "pronunciation": "/ˈæpl/",
        "partOfSpeech": "noun",
        "variants": ["apples"]
      }
    ],
    "total": 100,
    "page": 1,
    "limit": 20
  }
}
```

#### **GET /api/words/:wordId**
获取单词详情
```json
响应：
{
  "success": true,
  "data": {
    "wordId": "uuid",
    "word": "apple",
    "definition": "苹果",
    "rarity": "common",
    "pronunciation": "/ˈæpl/",
    "partOfSpeech": "noun",
    "variants": ["apples"],
    "collections": ["食物图鉴", "名词图鉴"]
  }
}
```

#### **GET /api/users/me/words**
获取当前用户的单词库存
```json
请求参数：
?isFavorited=true     // 只显示收藏的
&rarity=legendary     // 按稀有度筛选
&sortBy=obtainedAt    // 排序方式
&page=1
&limit=20

响应：
{
  "success": true,
  "data": {
    "words": [
      {
        "wordId": "uuid",
        "word": "apple",
        "definition": "苹果",
        "rarity": "common",
        "isFavorited": true,
        "obtainedAt": "2025-12-20T10:00:00Z"
      }
    ],
    "total": 50,
    "page": 1,
    "limit": 20
  }
}
```

#### **POST /api/users/me/words/:wordId/favorite**
收藏/取消收藏单词
```json
请求：
{
  "isFavorited": true
}

响应：
{
  "success": true,
  "data": {
    "wordId": "uuid",
    "isFavorited": true
  }
}
```

---

### **4. 卡包模块**

#### **GET /api/packs**
获取所有卡包列表
```json
响应：
{
  "success": true,
  "data": {
    "packs": [
      {
        "packId": "uuid",
        "name": "普通卡包",
        "description": "包含常见单词",
        "cardCount": 5,
        "weights": {
          "common": 70,
          "rare": 20,
          "epic": 8,
          "legendary": 2
        },
        "isActive": true
      }
    ]
  }
}
```

#### **GET /api/users/me/packs**
获取当前用户的卡包库存
```json
响应：
{
  "success": true,
  "data": {
    "packs": [
      {
        "packId": "uuid",
        "name": "普通卡包",
        "description": "包含常见单词",
        "count": 5
      }
    ]
  }
}
```

#### **POST /api/users/me/packs/:packId/open**
开卡包
```json
响应：
{
  "success": true,
  "data": {
    "drawnWords": [
      {
        "wordId": "uuid",
        "word": "apple",
        "definition": "苹果",
        "rarity": "common",
        "isNew": true  // 是否是新获得的单词
      },
      {
        "wordId": "uuid",
        "word": "dragon",
        "definition": "龙",
        "rarity": "legendary",
        "isNew": true
      }
    ],
    "remainingPacks": 4,
    "unlockedAchievements": [  // 可能解锁的成就
      {
        "achievementId": "uuid",
        "name": "单词收藏家",
        "rewardRoses": 50
      }
    ],
    "completedCollections": [  // 可能完成的图鉴
      {
        "collectionId": "uuid",
        "name": "动物图鉴",
        "rewardRoses": 100
      }
    ]
  }
}
```

---

### **5. 图鉴模块**

#### **GET /api/collections**
获取所有图鉴列表
```json
响应：
{
  "success": true,
  "data": {
    "collections": [
      {
        "collectionId": "uuid",
        "name": "动物图鉴",
        "description": "收集所有动物相关单词",
        "icon": "https://...",
        "rewardRoses": 100,
        "totalWords": 50
      }
    ]
  }
}
```

#### **GET /api/collections/:collectionId**
获取图鉴详情
```json
响应：
{
  "success": true,
  "data": {
    "collectionId": "uuid",
    "name": "动物图鉴",
    "description": "收集所有动物相关单词",
    "icon": "https://...",
    "rewardRoses": 100,
    "words": [
      {
        "wordId": "uuid",
        "word": "cat",
        "definition": "猫",
        "rarity": "common"
      }
    ],
    "totalWords": 50
  }
}
```

#### **GET /api/users/me/collections**
获取当前用户的图鉴完成进度
```json
响应：
{
  "success": true,
  "data": {
    "collections": [
      {
        "collectionId": "uuid",
        "name": "动物图鉴",
        "description": "收集所有动物相关单词",
        "totalWords": 50,
        "ownedWords": 48,
        "progress": 96.0,  // 百分比
        "isCompleted": false,
        "hasRewarded": false,
        "rewardRoses": 100
      },
      {
        "collectionId": "uuid",
        "name": "食物图鉴",
        "totalWords": 30,
        "ownedWords": 30,
        "progress": 100.0,
        "isCompleted": true,
        "hasRewarded": true,
        "completedAt": "2025-12-20T10:00:00Z"
      }
    ]
  }
}
```

---

### **6. 成就模块**

#### **GET /api/achievements**
获取所有成就列表
```json
请求参数：
?category=收集类      // 按类别筛选
&showHidden=false    // 是否显示隐藏成就

响应：
{
  "success": true,
  "data": {
    "achievements": [
      {
        "achievementId": "uuid",
        "name": "鲜花达人",
        "description": "累计获得100朵鲜花",
        "icon": "https://...",
        "category": "社交类",
        "isHidden": false,
        "conditionType": "total_roses",
        "conditionValue": 100,
        "rewardRoses": 50
      }
    ]
  }
}
```

#### **GET /api/users/me/achievements**
获取当前用户的成就进度
```json
响应：
{
  "success": true,
  "data": {
    "achievements": [
      {
        "achievementId": "uuid",
        "name": "鲜花达人",
        "description": "累计获得100朵鲜花",
        "icon": "https://...",
        "category": "社交类",
        "conditionType": "total_roses",
        "conditionValue": 100,
        "currentValue": 80,  // 当前进度
        "progress": 80.0,    // 百分比
        "isUnlocked": false,
        "rewardRoses": 50
      },
      {
        "achievementId": "uuid",
        "name": "初出茅庐",
        "description": "获得第一朵鲜花",
        "isUnlocked": true,
        "unlockedAt": "2025-12-15T10:00:00Z"
      }
    ]
  }
}
```

---

### **7. 签到模块**

#### **POST /api/users/me/signin**
每日签到
```json
响应：
{
  "success": true,
  "data": {
    "consecutiveDays": 5,
    "rewardRoses": 20,  // 基础10 + 连续奖励10
    "rewardPacks": [
      {
        "packId": "uuid",
        "name": "普通卡包",
        "count": 1
      }
    ],
    "totalRoses": 120,  // 签到后的总鲜花数
    "unlockedAchievements": [  // 可能解锁的成就
      {
        "achievementId": "uuid",
        "name": "坚持不懈",
        "rewardRoses": 50
      }
    ]
  }
}

错误响应（已签到）：
{
  "success": false,
  "error": "今天已经签到过了"
}
```

#### **GET /api/users/me/signin/status**
获取签到状态
```json
响应：
{
  "success": true,
  "data": {
    "hasSignedToday": true,
    "consecutiveDays": 5,
    "lastSignInAt": "2025-12-23T08:00:00Z",
    "nextReward": {
      "roses": 22,
      "packs": []
    }
  }
}
```

#### **GET /api/users/me/signin/history**
获取签到历史
```json
请求参数：
?page=1
&limit=30

响应：
{
  "success": true,
  "data": {
    "records": [
      {
        "day": 5,
        "rewardRoses": 20,
        "claimedAt": "2025-12-23T08:00:00Z"
      },
      {
        "day": 4,
        "rewardRoses": 18,
        "claimedAt": "2025-12-22T09:00:00Z"
      }
    ],
    "total": 30
  }
}
```

---

### **8. 排行榜模块**

#### **GET /api/leaderboard/roses**
鲜花排行榜
```json
请求参数：
?limit=100

响应：
{
  "success": true,
  "data": {
    "leaderboard": [
      {
        "rank": 1,
        "userId": "uuid",
        "username": "user1",
        "totalRoses": 1000,
        "level": 10
      },
      {
        "rank": 2,
        "userId": "uuid",
        "username": "user2",
        "totalRoses": 800,
        "level": 8
      }
    ],
    "myRank": {
      "rank": 50,
      "totalRoses": 100
    }
  }
}
```

#### **GET /api/leaderboard/vocab**
单词收藏排行榜
```json
响应：
{
  "success": true,
  "data": {
    "leaderboard": [
      {
        "rank": 1,
        "userId": "uuid",
        "username": "user1",
        "vocabCount": 500,
        "level": 10
      }
    ],
    "myRank": {
      "rank": 30,
      "vocabCount": 80
    }
  }
}
```

#### **GET /api/leaderboard/packs**
开包排行榜
```json
响应：
{
  "success": true,
  "data": {
    "leaderboard": [
      {
        "rank": 1,
        "userId": "uuid",
        "username": "user1",
        "totalPacks": 200,
        "level": 10
      }
    ],
    "myRank": {
      "rank": 40,
      "totalPacks": 50
    }
  }
}
```

#### **GET /api/leaderboard/consecutive**
连续签到排行榜
```json
响应：
{
  "success": true,
  "data": {
    "leaderboard": [
      {
        "rank": 1,
        "userId": "uuid",
        "username": "user1",
        "consecutiveDays": 30
      }
    ],
    "myRank": {
      "rank": 20,
      "consecutiveDays": 5
    }
  }
}
```

---

### **9. 消息模块（HTTP部分）**

#### **GET /api/messages**
获取历史消息（最近24小时）
```json
请求参数：
?before=timestamp     // 获取这个时间之前的消息
&limit=50             // 每次获取数量

响应：
{
  "success": true,
  "data": {
    "messages": [
      {
        "messageId": "uuid",
        "userId": "uuid",
        "username": "user1",
        "content": "Hello world",
        "roses": 5,
        "timestamp": 1703318400000,
        "replyTo": null,  // 引用的消息ID
        "hasRosed": false  // 当前用户是否已送花
      }
    ],
    "hasMore": true
  }
}
```

#### **GET /api/messages/:messageId**
获取单条消息详情
```json
响应：
{
  "success": true,
  "data": {
    "messageId": "uuid",
    "userId": "uuid",
    "username": "user1",
    "content": "Hello world",
    "roses": 5,
    "timestamp": 1703318400000,
    "replyTo": {
      "messageId": "uuid",
      "content": "Hi",
      "username": "user2"
    },
    "roseSenders": [  // 送花的人
      {
        "userId": "uuid",
        "username": "user3",
        "sentAt": "2025-12-23T10:00:00Z"
      }
    ]
  }
}
```

---

### **10. 管理员模块（可选）**

#### **POST /api/admin/users/:userId/ban**
封禁用户
```json
请求：
{
  "reason": "违规发言",
  "duration": 86400  // 封禁时长（秒），0表示永久
}

响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "isBanned": true,
    "bannedUntil": "2025-12-24T10:00:00Z"
  }
}
```

#### **POST /api/admin/users/:userId/unban**
解封用户
```json
响应：
{
  "success": true,
  "data": {
    "userId": "uuid",
    "isBanned": false
  }
}
```

#### **DELETE /api/admin/messages/:messageId**
删除消息
```json
响应：
{
  "success": true
}
```

#### **POST /api/admin/words**
添加新单词
```json
请求：
{
  "word": "serendipity",
  "definition": "意外发现珍奇事物的本领",
  "rarity": "legendary",
  "pronunciation": "/ˌserənˈdɪpəti/",
  "partOfSpeech": "noun",
  "variants": ["serendipities"]
}

响应：
{
  "success": true,
  "data": {
    "wordId": "uuid",
    "word": "serendipity",
    ...
  }
}
```

---

## 🔌 WebSocket 接口

### **连接地址：**
```
ws://localhost:3000/ws
或
wss://yourdomain.com/ws
```

### **连接时需要携带 Token：**
```javascript
const ws = new WebSocket('ws://localhost:3000/ws?token=jwt_token');
```

---

### **1. 客户端 → 服务器（发送消息）**

#### **事件：sendMessage**
发送聊天消息
```json
{
  "type": "sendMessage",
  "data": {
    "content": "Hello world",
    "replyTo": "message_uuid"  // 可选，引用的消息ID
  }
}
```

#### **事件：toggleRose**
送花/取消送花
```json
{
  "type": "toggleRose",
  "data": {
    "messageId": "uuid"
  }
}
```

#### **事件：typing**
用户正在输入
```json
{
  "type": "typing",
  "data": {
    "isTyping": true
  }
}
```

#### **事件：heartbeat**
心跳包（保持连接）
```json
{
  "type": "heartbeat",
  "data": {
    "timestamp": 1703318400000
  }
}
```

---

### **2. 服务器 → 客户端（接收消息）**

#### **事件：message**
新消息广播
```json
{
  "type": "message",
  "data": {
    "messageId": "uuid",
    "userId": "uuid",
    "username": "user1",
    "content": "Hello world",
    "roses": 0,
    "timestamp": 1703318400000,
    "replyTo": null
  }
}
```

#### **事件：roseUpdate**
送花更新
```json
{
  "type": "roseUpdate",
  "data": {
    "messageId": "uuid",
    "roses": 5,
    "action": "added",  // "added" 或 "removed"
    "senderId": "uuid",
    "senderUsername": "user2"
  }
}
```

#### **事件：userJoined**
用户上线
```json
{
  "type": "userJoined",
  "data": {
    "userId": "uuid",
    "username": "user1",
    "timestamp": 1703318400000
  }
}
```

#### **事件：userLeft**
用户下线
```json
{
  "type": "userLeft",
  "data": {
    "userId": "uuid",
    "username": "user1",
    "timestamp": 1703318400000
  }
}
```

#### **事件：onlineUsers**
在线用户列表更新
```json
{
  "type": "onlineUsers",
  "data": {
    "count": 15,
    "users": [
      {
        "userId": "uuid",
        "username": "user1"
      },
      {
        "userId": "uuid",
        "username": "user2"
      }
    ]
  }
}
```

#### **事件：userTyping**
有用户正在输入
```json
{
  "type": "userTyping",
  "data": {
    "userId": "uuid",
    "username": "user1",
    "isTyping": true
  }
}
```

#### **事件：notification**
系统通知
```json
{
  "type": "notification",
  "data": {
    "notificationType": "achievement",  // "achievement", "collection", "system"
    "title": "解锁新成就",
    "message": "恭喜你解锁成就：鲜花达人",
    "data": {
      "achievementId": "uuid",
      "name": "鲜花达人",
      "rewardRoses": 50
    }
  }
}
```

#### **事件：error**
错误消息
```json
{
  "type": "error",
  "data": {
    "code": "BANNED",
    "message": "你已被封禁"
  }
}
```

#### **事件：heartbeatAck**
心跳响应
```json
{
  "type": "heartbeatAck",
  "data": {
    "timestamp": 1703318400000
  }
}
```

---

## 📊 接口总结

### **HTTP 接口（共约 35 个）：**

**认证（4个）：**
- POST /api/auth/register
- POST /api/auth/login
- POST /api/auth/logout
- GET /api/auth/verify

**用户（3个）：**
- GET /api/users/me
- GET /api/users/:userId
- PUT /api/users/me

**单词（4个）：**
- GET /api/words
- GET /api/words/:wordId
- GET /api/users/me/words
- POST /api/users/me/words/:wordId/favorite

**卡包（3个）：**
- GET /api/packs
- GET /api/users/me/packs
- POST /api/users/me/packs/:packId/open

**图鉴（3个）：**
- GET /api/collections
- GET /api/collections/:collectionId
- GET /api/users/me/collections

**成就（2个）：**
- GET /api/achievements
- GET /api/users/me/achievements

**签到（3个）：**
- POST /api/users/me/signin
- GET /api/users/me/signin/status
- GET /api/users/me/signin/history

**排行榜（4个）：**
- GET /api/leaderboard/roses
- GET /api/leaderboard/vocab
- GET /api/leaderboard/packs
- GET /api/leaderboard/consecutive

**消息（2个）：**
- GET /api/messages
- GET /api/messages/:messageId

**管理员（4个，可选）：**
- POST /api/admin/users/:userId/ban
- POST /api/admin/users/:userId/unban
- DELETE /api/admin/messages/:messageId
- POST /api/admin/words

---

### **WebSocket 事件（共 12 个）：**

**客户端发送（4个）：**
- sendMessage（发送消息）
- toggleRose（送花/取消）
- typing（正在输入）
- heartbeat（心跳）

**服务器推送（8个）：**
- message（新消息）
- roseUpdate（送花更新）
- userJoined（用户上线）
- userLeft（用户下线）
- onlineUsers（在线用户）
- userTyping（有人输入）
- notification（系统通知）
- error（错误消息）
- heartbeatAck（心跳响应）

---

## 🎯 接口优先级建议

### **第一阶段（核心功能）：**
1. ✅ 用户认证（注册、登录）
2. ✅ WebSocket 聊天（发送消息、接收消息）
3. ✅ 送花功能（toggleRose）
4. ✅ 获取历史消息
5. ✅ 在线用户列表

### **第二阶段（单词系统）：**
6. ✅ 获取单词列表
7. ✅ 获取用户单词库存
8. ✅ 开卡包
9. ✅ 收藏单词

### **第三阶段（成就系统）：**
10. ✅ 图鉴进度
11. ✅ 成就列表
12. ✅ 每日签到

### **第四阶段（社交功能）：**
13. ✅ 排行榜
14. ✅ 用户资料
15. ✅ 邀请系统

### **第五阶段（管理功能）：**
16. ✅ 管理员接口
17. ✅ 数据统计

---

## 🔐 安全建议

### **1. 认证方式：**
- 使用 JWT Token
- Token 有效期：7天
- 支持 Token 刷新

### **2. 权限控制：**
- 普通用户只能操作自己的数据
- 管理员有额外权限
- WebSocket 连接需要验证 Token

### **3. 速率限制：**
- 发送消息：每分钟最多 20 条
- 送花：每分钟最多 30 次
- 开卡包：每分钟最多 10 次
- API 请求：每分钟最多 100 次

### **4. 数据验证：**
- 消息长度限制：500 字符
- 用户名长度：3-20 字符
- 密码强度：至少 8 位

---
